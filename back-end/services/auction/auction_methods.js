...
async function create_base_auction(client, item_name, item_description, user, start_time, auction_type, starting_amount, shipping_cost) {
  //use only within a transaction

  const auction_query = "INSERT INTO auctions (item_name, item_description, auction_owner, start_time, auction_type, starting_amount, shipping_cost) VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING *";
  const auction_query_values = [item_name, item_description, user.user_id, start_time, auction_type, starting_amount, shipping_cost];
...
async function create_dutch_auction(redis_client, pool_connection, auction_info, user) {
  const { item_name, item_description, auction_type, start_time, starting_amount, shipping_cost } = auction_info;
  if (!user) {
  ...
  try {
    ...
    const auction_query_result = await create_base_auction(client, item_name, item_description, user, unix_to_iso, auction_type, starting_amount, shipping_cost);
...
async function create_forward_auction(redis_client, pool_connection, auction_info, user) {
  const { item_name, item_description, auction_type, start_time, end_time, starting_amount, shipping_cost } = auction_info;
  if (!user) {
    throw new Error("user is undefined");
  }
  if (!end_time) {
